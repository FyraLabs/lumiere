...
namespace Lumiere {
    ...
    public class PlayerPage : Gtk.Box {
        private Lumiere.Widgets.BottomBar bottom_bar;
        ...
        construct {
            ...
            header_bar.header_bar.add_css_class ("top-bar");
            header_bar.valign = Gtk.Align.START;
            header_bar.add_css_class ("player-overlay");

            bottom_bar = new Widgets.BottomBar ();
            bottom_bar.valign = Gtk.Align.END;
            bottom_bar.add_css_class ("player-overlay");
            ...
            playback_manager.uri_changed.connect ((uri) => {
                // Title will be handled by the Window class
            });
        }

        private void update_actions_enabled (bool enabled) {
            unowned var application = Application.get_default ();
            ((SimpleAction) application.lookup_action (Lumiere.App.ACTION_NEXT)).set_enabled (enabled);
            ((SimpleAction) application.lookup_action (Lumiere.App.ACTION_PLAY_PAUSE)).set_enabled (enabled);
            ((SimpleAction) application.lookup_action (Lumiere.App.ACTION_PREVIOUS)).set_enabled (enabled);
        }
        ...
        private void reveal_control () {
            ...
            // Show controls
            set_cursor (null);
            header_bar.remove_css_class ("hidden");
            bottom_bar.remove_css_class ("hidden");
            ...
            hiding_timer = Timeout.add (2000, () => {
                ...
                header_bar.add_css_class ("hidden");
                bottom_bar.add_css_class ("hidden");
