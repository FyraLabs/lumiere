...
public class Lumiere.MprisPlayer : Object {
    [DBus (visible = false)]
    ...
    public string playback_status {
        get {
            var state = (bool) application.lookup_action (Lumiere.App.ACTION_PLAY_PAUSE).state;
            if (state == false) {
                return "Stopped";
            } else {
                return "Playing";
    ...
    construct {
        ...
        var action_next = application.lookup_action (Lumiere.App.ACTION_NEXT);
        action_next.bind_property ("enabled", this, "can-go-next", BindingFlags.SYNC_CREATE);

        var action_play_pause = application.lookup_action (Lumiere.App.ACTION_PLAY_PAUSE);
        action_play_pause.bind_property ("enabled", this, "can-play", BindingFlags.SYNC_CREATE);

        var action_previous = application.lookup_action (Lumiere.App.ACTION_PREVIOUS);
    ...
    public void next () throws GLib.Error {
        application.activate_action (Lumiere.App.ACTION_NEXT, null);
    }

    public void previous () throws GLib.Error {
        application.activate_action (Lumiere.App.ACTION_PREVIOUS, null);
    }

    public void play_pause () throws GLib.Error {
        application.activate_action (Lumiere.App.ACTION_PLAY_PAUSE, null);
    }
}
