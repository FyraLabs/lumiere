    ...
    public class LibraryManager : Object {
        ...
        private void create_video_object (File file) {
            ...
            if (file.get_parent ().get_path () != Environment.get_user_special_dir (UserDirectory.VIDEOS)) {
                ...
                shows[parent_name].add_item (new Lumiere.Objects.MediaItem.video (file.get_uri (), title, shows[parent_name]));
            } else {
                var item = new Lumiere.Objects.MediaItem.video (file.get_uri (), title, null);
        ...
        public Gdk.Pixbuf? get_poster_from_file (string poster_path) {
            ...
            if (FileUtils.test (poster_path, FileTest.EXISTS)) {
                try {
                    pixbuf = new Gdk.Pixbuf.from_file_at_scale (poster_path, -1, Lumiere.Services.POSTER_HEIGHT, true);
                } catch (Error e) {
                    warning (e.message);
                }
                if (pixbuf == null) {
                ...
                if (width > Lumiere.Services.POSTER_WIDTH) {
                    int x_offset = (width - Lumiere.Services.POSTER_WIDTH) / 2;
                    pixbuf = new Gdk.Pixbuf.subpixbuf (pixbuf, x_offset, 0, Lumiere.Services.POSTER_WIDTH, Lumiere.Services.POSTER_HEIGHT);
                }
            }
            return pixbuf;
        }
